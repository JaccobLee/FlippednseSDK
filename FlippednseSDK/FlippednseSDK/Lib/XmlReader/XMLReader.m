...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    NSString *strTextTemp = [dictInProgress objectForKey:@"text"];
    if (strTextTemp)
    {
        if (strTextTemp.length > 0)
        {
            [dictInProgress setObject:strTextTemp forKey:@"TextTemp"];
        }
    }
    ...
    if ([dictInProgress objectForKey:@"text"])
    {
        if ([[dictInProgress objectForKey:@"text"] length] > 0)
        {
            [dictInProgress removeObjectForKey:@"TextTemp"];
        }
    }
    
    if ([dictInProgress objectForKey:@"TextTemp"])
    {
        if ([[dictInProgress objectForKey:@"TextTemp"] length] > 0)
        {
            [dictInProgress setObject:[dictInProgress objectForKey:@"TextTemp"] forKey:@"text"];
            [dictInProgress removeObjectForKey:@"TextTemp"];
        }
    }
